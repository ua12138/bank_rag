...
def _tool_specs() -> list[dict[str, Any]]:
    return [
        {
            "name": "rag.health",
            "description": "Get runtime health and key configuration.",
...
def _call_tool(runtime: Runtime, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if name == "rag.list_documents":
        ...
        return {"documents": runtime.repo.list_documents(kb_id=kb_id, limit=limit)}
...
def build_mcp_app() -> FastAPI:
    app = FastAPI(
        title="HZ Bank RAG MCP Wrapper",
        version="0.1.0",
        description="MCP-style tool wrapper for agent integration.",
    )
        ...
        if req.method == "initialize":
            return _ok(
                req.id,
                {
                    ...
                    "serverInfo": {"name": "hz-bank-rag-mcp", "version": "0.1.0"},
                    "capabilities": {"tools": {}},
                },
            )
